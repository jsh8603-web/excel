...
_RULES = [
    ...
    #   "명세/내역/집계"는 검증형 metric_table 로 넘긴다(키워드 충돌 회피).
    ("listing", [r"정리표", r"정리해", r"목록", r"리스트", r"listing",
                 r"대장", r"보기\s*좋게", r"표로\s*정리", r"나열"]),
    # 일반 라벨+단일숫자 표(명세/내역/집계) — 필드계약 검증형. ★가장 마지막에 두어 모든
    #   구체 분석형(variance/fc_*/investment_appraisal/headcount 등)이 먼저 이긴다.
    #   ★프로파일 키워드(상각비·물량·투자비)는 *예시*일 뿐 — mixed+임의 FieldSpec 으로
    #   어떤 단일숫자(거리·온도·건수 등)든 받는 범용형. 무매칭 시 하드 기본값은 pnl(불변).
    ("metric_table", [r"명세서", r"명세\b", r"내역서", r"내역\b", r"집계표", r"집계\b",
                      r"항목별", r"항목\s*금액", r"단가표", r"금액표", r"수량표",
                      r"원가표", r"원가\s*명세", r"라벨.*숫자", r"단일\s*숫자",
                      r"line[\s-]*item", r"라인\s*아이템", r"항목\s*표"]),
]
