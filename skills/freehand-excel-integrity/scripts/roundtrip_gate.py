#!/usr/bin/env python3
"""
roundtrip_gate.py — 워크북 변형(mutation)이 '연결'을 끊지 않았는지 보증하는 무결성 게이트.
...
함께 지문화해 before↔after 를 비교한다. 의존성: openpyxl + stdlib (fpna/ 무관, 이식 가능).

이 스크립트는 freehand-excel-integrity 스킬과 함께 이동한다. xlwings live-edit·pywin32
COM 처럼 **기존 워크북을 제자리 변형(mutation)** 하는 백엔드의 필수 검증이다 — openpyxl
authoring(처음부터 작성)은 잃을 부품이 없으므로 xlsx_doctor 만으로 충분하지만, 제자리
변형은 이 게이트로 부품 보존을 증명해야 한다.

쓰임:
  python3 scripts/roundtrip_gate.py <before.xlsx> <after.xlsx>     # 직접 비교
  from roundtrip_gate import guard; guard(path, mutate_fn)          # 변형을 감싸 보증
Exit: 0 = 무결, 1 = 끊김(고아화) 검출.
"""
from __future__ import annotations
import sys, os, zipfile, argparse
# vendored openpyxl 이 있으면 사용(jsh8603-web/excel repo 안), 없으면 설치본(fixed_trans 등).
