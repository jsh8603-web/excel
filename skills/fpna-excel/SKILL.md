...
## 2.6 일반표 — 라벨+단일숫자 / 정리표 (분석형이 아닐 때의 풍부한 기본형)

분석모형이 아닌 "항목(글자)+숫자 한두 개" 명세/내역/집계가 대부분이다. pnl 기본값으로
떨구지 않고 두 범용형으로 받는다(모든 구체 템플릿이 먼저 이긴 뒤, 무매칭 하드 기본값 pnl 전):
- **`metric_table`** (명세/내역/집계/항목표·단가표·원가표): 필드 단위 **계약 검증**(not_null·
  범위·부호·허용값·tie-out). 결측=NO_DATA 노출(저장 OK)/계약위반=VIOLATION(저장 차단).
  ★프로파일(물량·상각비·투자비·임금)은 **예시 프리셋**일 뿐 — `mixed`+임의 `FieldSpec` 으로
  거리 km·온도 ℃·건수·점수 등 **재무·비재무 어떤 단일숫자든** 받는 범용 템플릿.
- **`listing`** (정리표/목록/리스트): 계산 없는 정리표(그룹소계·총계) + fidelity(행보존·소계 tie).
둘 다 `run_report` 스파인 통과 — 계약 위반·은폐 시 저장 차단.
